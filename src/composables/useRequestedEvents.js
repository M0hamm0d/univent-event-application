  ...
  const fetchRequestedAndEvents = async (page = 1, filters = {}) => {
    ...
    try {
      const { data: requested_event, error: reqError } = await supabase
        ...
        .select(
          'id,created_at, event_title, category, time, date,description,location,image_url, price,free_or_paid, link_to_register, end_date, capacity, event_format,requires_registration,user_id,user_email',
        )
