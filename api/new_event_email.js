...
export default async function handler(req, res) {
  ...
  try {
    ...
    const uniqueUsers = Array.from(new Map(users.map((u) => [u.user_email, u])).values())
    ...
    await Promise.all(
      uniqueUsers.map(async (user) => {
        try {
          // Parse user interested events safely
          let categories = []
          if (typeof user.interested_events === 'string') {
            try {
              categories = JSON.parse(user.interested_events)
          ...
          // Filter events matching user's interests
          const matchedEvents = events.filter((event) => {
          ...
          if (!matchedEvents.length) {
            result.push({ email: user.user_email, matchedEvents: 0, reason: 'no matches' })
            console.log(`User ${user.user_email}: no matching events`)
            return
          }

          // Build HTML content
          const htmlContent = `
            <h1>New Events Just for You! 🎉</h1>
            <p>Hi there! Based on your interests, we found some new events you might love:</p>
            ${matchedEvents
              .map(
                (event) => `
          ...
          try {
            ...
            console.error(
              `Failed to send email to ${user.user_email}:`,
